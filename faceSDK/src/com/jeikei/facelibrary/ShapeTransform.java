...
public class ShapeTransform {
	...
	*/
	private void shutMouthUp()
	{
		...
		for(int i=55; i<63; i++)	//입을 벌리고 있기 때문에 입 크기 조절
		{
	...
	private void completeModelShape()
	{
		...
		for(int i=0; i<modelShapeX.length; i++)
		{
			...
			////모델 shape에 지정되지 않은 값 검사 (x=0, y=0)
		...
		///위에서 구한 지정되지 않은 값을 얼굴에서 구한 포인트로 채워넣음
		for(int i=0; i<modelShapeX.length; i++)
		{
	...
	private void centerTransform()
	{
		...
		_c_eye = (x[34]+x[29])/2;		////////////// 눈 중심점(미간) 구하기
		_c_lips = (x[51]+x[61])/2;		//입 중심점
		_c_nose = x[41];
